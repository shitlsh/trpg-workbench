...
**建议落地方式：**
- [x] 直接改代码：`AgentPanel` 在 `thinking` 非空时记录 `thinkingStartedAt`，`isStreaming` 变为 `false` 后计算时长，传入 `ThinkingBlock` 作为可选 `duration` prop，在折叠的 header 中显示"推理 Xs"
...
**建议落地方式：**
- [x] 直接改代码：后端 `director.py` 落库时在 content 中插入 `{{tool:id}}` 占位符；前端 `StoredMessageBubble` 按占位符分段渲染
...
**建议落地方式：**
- [x] 直接改代码：`AgentPanel.tsx` 中 `StoredMessageBubble` 和 `StreamingBubble` 的 AI 回复容器去掉气泡样式

...
- [x] `apps/backend/app/services/chat_service.py`：`trim_to_budget()` 触发截断时返回一个 flag（如 `truncated: bool`）
- [x] `apps/backend/app/api/chat.py`：在发送给 director 之前，若 `truncated=True`，在历史消息队列头部插入一条 `system` 角色消息："[系统提示：对话历史较长，部分早期内容已移出上下文窗口]"
- [x] 前端：历史中出现 `role=system` 的截断提示消息时，以浅色分割线样式区别于普通气泡展示

**Phase 2（中改，需额外 LLM 调用）：**
- [x] 截断前调用轻量 LLM 生成历史摘要（≤200字），将摘要作为 system 消息注入替代被丢弃的原始轮次
- [x] 摘要生成失败时降级到 Phase 1 的静默截断+提示

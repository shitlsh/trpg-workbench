...
recommended_action: defer_or_structured_alternative
---
...
| 动作 | 说明 |
|------|------|
| `proposed` → `accepted` | 若团队同意「不默认可变 shell、优先结构化批处理」作为正式原则 |
| `proposed` → `deferred` | 若需等产品上大量批量整理反馈再开专项 milestone |
| `proposed` → `rejected` | 仅当明确决定**永不**提供任何形式命令能力时（一般不推荐把话说死；可 deferred） |
| 新 milestone | 若接受「批处理 + 预览」路线，单独立项：工作区批量替换/移动 API + Director 工具 +（可选）UI diff 预览 |

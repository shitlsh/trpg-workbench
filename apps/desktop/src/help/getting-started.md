...
规则集是 TRPG 创作的核心组织单元，包含知识库和创作风格提示词。可以选择内置的「通用」规则集作为起点，也可以在向导完成后再到 [规则集](/settings/rule-sets) 页面创建专属规则集。不确定时直接点击「跳过此步骤」。
...
| 入口 | 用途 | 帮助文档 |
|------|------|---------|
| [规则集](/settings/rule-sets) | 管理规则集、知识库和创作风格提示词 | [规则集管理](/help/rule-set-management) |
...
- **想导入规则书？** 前往 [规则集](/settings/rule-sets) 选择规则集，在知识库区域新建并上传 PDF。详见 [知识库导入指南](/help/knowledge-import)。

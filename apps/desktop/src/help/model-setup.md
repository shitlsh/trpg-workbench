...
3. 选择供应商，按需填写 Base URL（OpenAI Compatible / OpenRouter 需要）
4. 填写 API Key，点击「验证 Key」按钮 — 验证成功后会自动拉取可用模型列表
5. 从「选择模型」下拉中选择你想使用的模型，配置名称会自动建议，也可手动修改
6. 点击「保存」完成配置

> **提示：** 验证通过后选好的模型会在创建工作空间时自动预填到「模型路由」，无需重复选择。如果跳过验证直接保存，也可在 [工作空间设置](/help/start-creating) 的「模型路由」里随时指定或切换模型。
...
>
> 具体量化格式（如 Q4_K_M）由运行软件自动推荐，一般选默认即可。
...
3. 回到 TRPG Workbench，在配置向导或模型配置页面：
   ...
   - 模型名称：填入 LM Studio 中显示的模型 ID（如 `lmstudio-community/Qwen3-14B-GGUF`）
   - API Key：填写任意字符（如 `lm-studio`）或留空

**使用 Ollama：**

1. 安装 Ollama，执行 `ollama pull qwen3:14b` 下载模型
2. Ollama 默认自动运行服务（端口 11434）
3. 在 TRPG Workbench 中：
   - 供应商选择：`OpenAI Compatible（含本地模型）`
   - Base URL：`http://localhost:11434/v1`
   - 模型名称：`qwen3:14b`（与 pull 时名称一致）
...
Embedding 模型同样可以本地运行，无需联网。常见可用选项如 `nomic-embed-text` 系列在 LM Studio 和 Ollama 中均有支持。
...
- 工具调用与「类 JSON 结构化」输出能力因**具体模型**而异；在绑定工作区默认 LLM 时，模型选择器会按需筛掉明确标为不支持的项
...
1. 进入工作空间的 [设置](/help/start-creating) 页面
2. 在「模型路由」中为工作区选择**默认 LLM 配置**（LLM Profile），并选择**具体模型名称**；列表支持搜索与按能力筛选

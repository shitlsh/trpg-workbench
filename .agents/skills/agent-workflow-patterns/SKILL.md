...
```json
{
  ...
  "status": "pending | planning | waiting_for_clarification | executing | paused | awaiting_confirmation | completed | failed",
  "current_step": 4,
  "total_steps": 13,
  "input_snapshot": {
  ...
  "step_results": [
    {"step": 1, "name": "读取 Workspace 配置", "status": "completed", "summary": "...", "detail": null, "error": null},
    {"step": 2, "name": "生成变更计划", "status": "waiting_confirm", "summary": "...", "detail": null, "error": null},
    {"step": 3, "name": "检索规则知识库", "status": "completed", "summary": "规则检索完成", "detail": "<citations JSON string>", "error": null}
  ],
...
pending
  └─► planning                  （Director 开始分析）
        ...
        │     └─► paused → (user confirms plan) → running
        └─► paused              （信息充分，直接等待用户确认计划）
              └─► running       （用户确认后恢复执行）
                    └─► completed | failed
```
...
- RAG 检索从 Step 3（Rules Agent）开始，此时 `clarification_answers` 已就位
- `clarification_answers` 可作为检索 query 增强传入 Rules Agent

...
```
retrieve（top_n） → rerank → top_k → 注入 Agent / 返回结果
```

- rerank 必须通过配置开关显式启用，默认不激活
- 启用 rerank 的前提：已完成知识库预览与质量检查，确认基础文本提取质量和 citation 可靠性达标
...
- rerank provider 配置独立于 EmbeddingProfile，挂载在 WorkspaceRAGConfig 或全局 RAGConfig
...
**rerank 已纳入当前版本实现范围（M8 A2），但默认不启用。** 需通过 WorkspaceRAGConfig 配置开关显式开启。

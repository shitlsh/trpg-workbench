...
```json
{
  "chunk_id": "chunk_xxx",
  "content": "相关片段文本",
  "document_name": "coc7e_core_rules.pdf",
  "page_from": 45,
  "page_to": 46,
...
- `document_name`：文档名称字段（M12 修正，前身为 `document_filename`，现统一使用 `document_name`）
...
### M12：citations 写入 WorkflowStepResult.detail

检索步骤完成后，Workflow 必须将 citations 列表序列化为 JSON 字符串写入 `WorkflowStepResult.detail`：
...
} for c in citations], ensure_ascii=False)

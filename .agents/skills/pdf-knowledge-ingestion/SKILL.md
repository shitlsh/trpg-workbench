...
- 第一版不引入 rerank，直接以向量相似度排序结果
- rerank 作为后续可扩展增强步骤，在启用前应先保证文本提取质量、chunk 质量和 citation 可靠性
...
| rerank | 在 `retrieve → rerank → top_k` 中作为可插拔增强步骤，提升多库联合检索和创作型查询的排序质量 |

**启用 rerank 的前提**：应先完成知识库预览与质量检查，确认文本提取质量和 citation 可靠性达标，再考虑引入 rerank 解决"召回到了但排序不够准"的问题。

...
**精简原则**（同一版本内多个 commit 围绕同一主题时）：

- 围绕同一功能模块的多个 feat/fix commit → **合并为一条**，用主题命名（如"问答交互优化"而非逐条列举）
- 描述聚焦于用户感知的**最终结果**，过程中的中间修复（如"修复重复渲染""修复时序问题"）归并到结果描述中，不单独列出
- 条目数量宁少勿多：一个版本 1-3 条 Feature + 1-3 条 Bug Fix 是健康范围，超过时优先合并同主题条目

...
**A4：skill_agent.py 调用路径确认与修复**

`skill_agent.py` 是 M17 实现的"通过 chat 创建 Agent Skill"功能的后端组件。M19 重构可能断开其调用路径。

方案：
- 检查 `api/chat.py` 或其他入口是否仍调用 `run_skill_agent`
- 若调用路径已断：将 `skill_agent.py` 接回 Director 工具链，新增 `create_skill(user_intent)` 工具
- 若已通过其他路径正常工作：保持不变，只确认有测试覆盖
- 若 M17 的 Skill 功能整体已废弃：标记 `skill_agent.py` 为待删除，在本 milestone 内清理
...
apps/backend/app/agents/skill_agent.py  — 待确认（A4 分支决策）
```

### 修改文件

```
apps/backend/app/agents/tools.py                  — 新增 check_consistency、consult_rules（及可能的 create_skill）；search_assets 语义路径；写入工具信任模式逻辑
...
### A4：skill_agent.py 调用路径确认

- [ ] **A4.1**：检查 `api/chat.py` 及所有 API 文件，确认 `run_skill_agent` 当前是否有调用路径
- [ ] **A4.2（分支 A）**：若调用已断 → `tools.py` 新增 `create_skill` 工具，接回主流程
- [ ] **A4.2（分支 B）**：若已通过其他路径正常工作 → 无需改动，记录确认结论
- [ ] **A4.2（分支 C）**：若 M17 Skill 功能整体废弃 → 删除 `skill_agent.py` 及对应 prompts
...
9. `consistency.py` / `rules.py` 有明确调用路径（通过 `check_consistency` / `consult_rules` 工具），不再是死代码

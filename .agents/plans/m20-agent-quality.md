...
- `tools.py` 新增 `create_skill(user_intent)` 工具：
  ...
  - 调用 `run_skill_agent(user_intent, knowledge_ctx, workspace_context, model)`
  - 生成 Skill 文件的 PatchProposal（写入工作空间 `skills/` 目录），raise `PatchProposalInterrupt`
- `skill_agent.py` 保留，接口调整：model 从 `_workspace_context` 解析传入

...
- [x] **A1.1**：`apps/backend/app/agents/tools.py` — 定义 `AgentQuestionInterrupt(Exception)` 类，含 `questions: list[dict]` 属性
- [x] **A1.2**：`apps/backend/app/agents/tools.py` — 实现 `ask_user` 工具函数，校验 questions 格式，抛出 `AgentQuestionInterrupt`
- [x] **A1.3**：`apps/backend/app/agents/tools.py` — 将 `ask_user` 加入 `ALL_TOOLS` 列表

### A2：director.py SSE 捕获

- [x] **A2.1**：`apps/backend/app/agents/director.py` — 在 `run_director_stream` 的 `except` 块前捕获 `AgentQuestionInterrupt`
- [x] **A2.2**：`apps/backend/app/agents/director.py` — yield `agent_question` SSE 事件（含 id + questions），再 yield `done`

### A3：system prompt 约束

- [x] **A3.1**：`apps/backend/app/prompts/director/system.txt` — 在工具调用规范章节末尾追加 `ask_user` 使用约束段落

### A4：shared-schema 类型

- [x] **A4.1**：`packages/shared-schema/src/index.ts` — 新增 `AgentQuestionOption`、`AgentQuestionItem`、`AgentQuestion` 接口
- [x] **A4.2**：`packages/shared-schema/src/index.ts` — 在 `SSEEvent` 联合类型中追加 `agent_question` 分支

### A5：前端 QuestionCard

- [x] **A5.1**：`apps/desktop/src/components/agent/QuestionCard.tsx` — 实现 QuestionCard 组件（选项按钮、单/多选、只读态、提交回调）
- [x] **A5.2**：`apps/desktop/src/components/agent/AgentPanel.tsx` — StreamEvent 联合类型追加 `question_interrupt` 分支
- [x] **A5.3**：`apps/desktop/src/components/agent/AgentPanel.tsx` — SSE 解析时处理 `agent_question` 事件，push `question_interrupt` 到 streamingEvents
- [x] **A5.4**：`apps/desktop/src/components/agent/AgentPanel.tsx` — `StreamingBubble` 中渲染 `question_interrupt` 事件为 `QuestionCard`
- [x] **A5.5**：`apps/desktop/src/components/agent/AgentPanel.tsx` — `QuestionCard.onSubmit` 回调：拼装答复文本，调用 `handleSend()` 自动发送

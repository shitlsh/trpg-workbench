...
- `knowledge-library-ingest/SKILL.md`（曾用名 `pdf-knowledge-ingestion`）：更新知识库创建流程（必须指定 rule_set_id）

**A6：清理 pre-1.0 技术债**

方案（趁本次 schema 变更一起清理，删库重建无额外成本）：
- 删除 `database.py` 中的 `_migrate_add_column()` 手写迁移系统——直接在 ORM 定义中声明正确 schema，不再运行时 patch
- 删除 `KnowledgeLibraryORM.embedding_config` M2 遗留字段（ORM、Python schema、shared-schema 三层同步删除），已被 `embedding_profile_id` 取代
...
```
# 后端
apps/backend/app/models/orm.py                — rule_set_id NOT NULL, 删 RuleSetLibraryBindingORM, 删 embedding_config
apps/backend/app/models/schemas.py             — 删 RuleSetLibraryBinding schema, rule_set_id 改必填, 删 embedding_config
...
apps/backend/app/storage/database.py           — 删除 _migrate_add_column 手写迁移
packages/shared-schema/src/index.ts            — 删 RuleSetLibraryBinding 类型, 删 embedding_config, 更新 CreateKnowledgeLibraryRequest
...
.agents/skills/knowledge-library-ingest/SKILL.md
```
...
- [x] **A5.4**（已在 M15 前更新，无 binding 残留）：`knowledge-library-ingest/SKILL.md` — 更新知识库创建流程
...
11. 后端无 `_migrate_add_column` 手写迁移逻辑
12. ORM/schema/shared-schema 中无 `embedding_config` M2 遗留字段

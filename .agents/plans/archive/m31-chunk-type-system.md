...
- [x] **A1.1**：`app/knowledge/types.py` — 将 enum 改为 `RULE / ENTITY / LORE / ADVENTURE / APPENDIX / NONE`，删除 `EXAMPLE / TABLE / PROCEDURE / FLAVOR`
- [x] **A1.2**：`app/knowledge/types.py` — 更新 `RULE_CHUNK_TYPES = ["rule", "entity"]` 和 `LORE_CHUNK_TYPES = ["lore", "adventure"]`

### A2：添加 `build_chunk_types_section()`

- [x] **A2.1**：`app/knowledge/types.py` — 实现 `build_chunk_types_section()` 函数，输出标准中文描述段落，包含每个类型的中英文名、含义说明、`none` 的兜底语义说明

### A3：更新 toc_analyzer

- [x] **A3.1**：`app/prompts/toc_analyzer/system.txt` — 更新 `suggested_chunk_type` 枚举注释为新的 5 种
- [x] **A3.2**：`app/prompts/toc_analyzer/chm_classify_system.txt` — 更新 CHM 分类枚举注释为新的 5 种
- [x] **A3.3**：`app/knowledge/toc_analyzer.py` — `parse_pdf_toc_response` 中的 `valid_chunk_types` 改为从 `types.py` 导入（`{t.value for t in ChunkType}`）
- [x] **A3.4**：`app/knowledge/toc_analyzer.py` — CHM 对应的 `valid` set 同样改为从 `types.py` 导入

### A4：注入 Director system prompt

- [x] **A4.1**：`app/agents/director.py` — `build_director_prompt()` 中调用 `build_chunk_types_section()` 并注入（与 `_build_asset_types_section()` 同位置）

### A5：更新 consult_rules 工具描述

- [x] **A5.1**：`app/agents/tools.py` — 在 `consult_rules` 工具定义中补充 `RULE_CHUNK_TYPES` 语义说明，注明 `none` 类型作为兜底的行为

### A6：排查其他 prompt 文件

- [x] **A6.1**：搜索 `app/prompts/` 全目录，无其他文件引用旧 chunk type 值；同步更新了 `director/system.txt`（补充 `consult_lore` 工具条目与知识库工具选择指引）和 `search_knowledge` 工具描述

### A7：同步 TypeScript 类型

- [x] **A7.1**：`packages/shared-schema/src/index.ts` — 更新 `ChunkType` union 为 `"rule" | "entity" | "lore" | "adventure" | "appendix" | "none"`

### A8：修正 retriever.py none 类型兜底

- [x] **A8.1**：`app/knowledge/retriever.py` — 修正 type_filter 逻辑，`chunk_type == "none"` 与 `chunk_type == null` 等价处理（`effective_type` 计算后再过滤）

### A9：新增 consult_lore 工具

- [x] **A9.1**：`app/agents/tools.py` — 实现 `consult_lore` 工具，使用 `LORE_CHUNK_TYPES = ["lore", "adventure"]` 过滤，面向世界观/剧情类检索
- [x] **A9.2**：`app/agents/tools.py` — 将 `consult_lore` 注册到 `ALL_TOOLS` 和 `EXPLORE_TOOLS`

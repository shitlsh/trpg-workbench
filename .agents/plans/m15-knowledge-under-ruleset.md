...
方案：
- `KnowledgeLibrary.rule_set_id` 改为 NOT NULL（SQLite migration：先为所有 NULL 行补上默认 rule_set_id，再加 NOT NULL 约束）
...
# 迁移步骤：
# 1. 为所有 rule_set_id IS NULL 的知识库分配默认规则集
#    - 如果只有一个规则集，直接绑定
#    - 如果有多个，创建一个"未分类"规则集
# 2. 重建 knowledge_libraries 表，rule_set_id NOT NULL
# 3. 迁移 rule_set_library_bindings 数据到 rule_set_id FK（如有不一致）
# 4. 删除 rule_set_library_bindings 表
```
...
- [ ] **A1.6**：数据迁移脚本 — 处理现有 NULL `rule_set_id` 数据
- [ ] **A1.7**：Agent 运行时知识库获取逻辑更新（如 `workspace_context` 相关代码）
